...
RandomExcursions(int n, BitSequence *epsilon)
{
    ...
    //
    results[TEST_RND_EXCURSION] = stats[TEST_RND_EXCURSION] = stdout;

	if ( ((S_k = (int *)calloc(n, sizeof(int))) == NULL) ||
		 ((cycle = (int *)calloc(MAX(1000, n/100), sizeof(int))) == NULL) ) {
        fprintf(stats[TEST_RND_EXCURSION], "Random Excursions Test:  Insufficent Work Space Allocated.\n");
		if ( S_k != NULL )
			free(S_k);
		if ( cycle != NULL )
			free(cycle);
	...
	for( i=1; i<n; i++ ) {
		...
		if ( S_k[i] == 0 ) {
			J++;
			if ( J > MAX(1000, n/100) ) {
                fprintf(stats[TEST_RND_EXCURSION], "ERROR IN FUNCTION randomExcursions:  EXCEEDING THE MAX NUMBER OF CYCLES EXPECTED\n.");
				free(S_k);
				free(cycle);
                return FLAG_FAILURE;
			}
    ...
    //
    if(isNegative(p_value) || isGreaterThanOne(p_value) || p_value < ALPHA)
    {
        return FLAG_FAILURE;
    }
    return FLAG_SUCCESS;
