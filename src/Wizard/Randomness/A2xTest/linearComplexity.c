...
LinearComplexity(int M, int n, BitSequence *epsilon)
{
    ...
    //
    results[TEST_LINEARCOMPLEXITY] = stats[TEST_LINEARCOMPLEXITY] = stdout;
	...
	if ( ((B_ = (BitSequence *) calloc(M, sizeof(BitSequence))) == NULL) ||
        ...
        fprintf(stats[TEST_LINEARCOMPLEXITY], "Insufficient Memory for Work Space:: Linear Complexity Test\n");
		if ( B_ != NULL )
			free(B_);
		if ( C != NULL )
			free(C);
    ...
    fprintf(stats[TEST_LINEARCOMPLEXITY], "chi2 = %9.6f\t%s\tp_value = %9.6f\n", chi2, p_value < ALPHA ? "FAILURE" : "SUCCESS", p_value); fflush(stats[TEST_LINEARCOMPLEXITY]);
