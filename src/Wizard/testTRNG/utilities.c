...
//#include "stat_fncs.h"
#include "Randomness/A2xTest/A2xTest.h"
...
nist_test_suite()
{
	if ( (testVector[0] == 1) || (testVector[TEST_FREQUENCY] == 1) ) 
        Frequency(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_BLOCK_FREQUENCY] == 1) ) 
        BlockFrequency(tp.blockFrequencyBlockLength, tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_CUSUM] == 1) )
        CumulativeSums(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RUNS] == 1) )
        Runs(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_LONGEST_RUN] == 1) )
        LongestRunOfOnes(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RANK] == 1) )
        Rank(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_FFT] == 1) )
        DiscreteFourierTransform(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_NONPERIODIC] == 1) )
        NonOverlappingTemplateMatchings(tp.nonOverlappingTemplateBlockLength, tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_OVERLAPPING] == 1) )
        OverlappingTemplateMatchings(tp.overlappingTemplateBlockLength, tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_UNIVERSAL] == 1) )
        Universal(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_APEN] == 1) )
        ApproximateEntropy(tp.approximateEntropyBlockLength, tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RND_EXCURSION] == 1) )
        RandomExcursions(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RND_EXCURSION_VAR] == 1) )
        RandomExcursionsVariant(tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_SERIAL] == 1) )
        Serial(tp.serialBlockLength,tp.n, epsilon);
	
	if ( (testVector[0] == 1) || (testVector[TEST_LINEARCOMPLEXITY] == 1) )
        LinearComplexity(tp.linearComplexitySequenceLength, tp.n, epsilon);
}
