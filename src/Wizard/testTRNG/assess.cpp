...
main(int argc, char *argv[])
{
    printf("Hello World! \r\n ");
    {
        CRandomness oRandomness;
        int iRet = oRandomness.TestRandomnessExport(0x01);
        {
            printf("call CPP TestRandomnessExport = %d.\n", iRet);
        }
        iRet = TestRandomnessExport(0x02);
        {
            printf("call C TestRandomnessExport = %d.\n", iRet);
        }
    }
    ...
    //8
    begin = clock();
    ret = BinaryDerivate(3, 10000, rand_bits);
    end = clock();
...
computeMetrics(char *s, int test)
{
    ...
    qsort((void *)A, sampleSize, sizeof(double), (__compar_fn_t)cmp);
	for ( j=0; j<sampleSize; j++ ) {
	...
	if ( expCount == 0 )
        ...
        uniformity = cephes_igamc((double)9.0/2.0, (double)chi2/2.0);
	}
